...
export class Client extends EventEmitter {
    ...
    constructor(options: { id: string; agent: TunnelAgent; }) {
        ...
        this.debug = Debug(`x-tunnel:Client[${this.id}]`);
    ...
    handleUpgrade(req: http.IncomingMessage, socket: Socket | Duplex) {
        this.debug('> [up] %s', req.url);
        socket.once('error', (err: NetError) => {
        ...
        this.agent.createConnection({}, (err: any, conn: Socket) => {
            this.debug('< [up] %s', req.url);
